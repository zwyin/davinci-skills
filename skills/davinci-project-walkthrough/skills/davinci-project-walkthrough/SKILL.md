---
name: davinci-project-walkthrough
version: "1.6.4"
...
```
/davinci-project-walkthrough [path] [--depth brief|medium|deep|all] [--audience general|dev] [--lang zh|zh-pure|en|bilingual] [--no-confirm] [--version]
```
...
2. Tokens starting with `--` are flags:
   ...
   - `--version` is a standalone flag (no value consumed); when present, print `project-walkthrough v1.6.4` and exit immediately
...
7. If `--version` is present, print `project-walkthrough v1.6.4` and stop — ignore all other flags and path
...
- `--version` (optional) — Print version (`project-walkthrough v1.6.4`) and exit.

Invalid or missing flag values fall back to defaults. If a flag is repeated, the last occurrence wins.

**Examples:**
```
/davinci-project-walkthrough                                              # Brief + general + zh (CWD)
/davinci-project-walkthrough /path/to/project                             # Brief + general + zh
/davinci-project-walkthrough /path/to/project --depth medium              # Medium + general + zh
/davinci-project-walkthrough /path/to/project --depth deep --audience dev # Deep + dev + zh
/davinci-project-walkthrough --depth deep                                 # Deep + general + zh (CWD)
/davinci-project-walkthrough --audience dev                               # Brief + dev + zh (CWD)
/davinci-project-walkthrough --depth all --audience dev                   # All depths + dev + zh (CWD)
/davinci-project-walkthrough ./deep --depth brief                         # Path "./deep", brief + general + zh
/davinci-project-walkthrough /path/to/project --lang en                   # Brief + general + en
/davinci-project-walkthrough --depth deep --lang en                       # Deep + general + en (CWD)
/davinci-project-walkthrough --depth deep --no-confirm                    # Deep + skip confirmation
/davinci-project-walkthrough /path/to/project --lang zh-pure              # Brief + general + zh-pure
/davinci-project-walkthrough /path/to/project --lang bilingual            # Brief + general + bilingual
/davinci-project-walkthrough --version                                    # Print version and exit
```
...
- `<prefix>` — the brand prefix, first segment of the `name` field in frontmatter (e.g., derived from `davinci-project-walkthrough`)
...
If the script outputs an upgrade reminder, inform the user: "A new version is available. You can continue with the current version, or run `claude plugin update davinci-skills@davinci-skills` to upgrade."
...
**Startup:** Print `Project Walkthrough v1.6.4` before any analysis. This lets users confirm which version they're running.
...
3. Run the converter tool as the primary HTML generation method:
   ...
   - `--tool-name <name>`: tool name shown in the promo page (default: `davinci-project-walkthrough`)
